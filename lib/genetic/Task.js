...
function Task(options) {
  ...
  this.elitism = false || options.elitism
}
...
Task.prototype.reproduction = function (callback) {
  ...
  async.series([
      ...
      , function (callback) {
        ...
        sum = 0 // https://github.com/dolphin278/genetic/pull/7
        ...
        if (self.minimize) {
          async.forEach(self.parents
            , function (item, cb) {
              item.score*=-1
              item.score+=maxScore
              sum+=item.score; // https://github.com/dolphin278/genetic/pull/7
...
Task.prototype.childSelection = function (callback) {
  ...
  self.population = []
  if (self.elitism) {
    // clone it, whatever it is
    // let max = JSON.parse(JSON.stringify(self.statistics.max))
    // let min = JSON.parse(JSON.stringify(self.statistics.min))
    // remove the score to be consistent  ?
    // causes error when minimize is true
    // delete max.score
    self.population.push(self.minimize ? JSON.parse(JSON.stringify(self.statistics.min)) : JSON.parse(JSON.stringify(self.statistics.max)) )
  }
